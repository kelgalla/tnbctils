...
use Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(uuidToBarcode uuidSampleType getBiotab getFields uuidToData uuidToBarcodeFull);
...
#print($url . "\n\n");
    ...
    my $hits;
    if ($legacy) {
	my $cmd = "curl --request POST --header \"Content-Type: application/json\" --data '$json' 'https://api.gdc.cancer.gov/legacy/files' 2> /dev/null";
	my $resjson = qx/$cmd/;
	#print($resjson . "\n\n");
